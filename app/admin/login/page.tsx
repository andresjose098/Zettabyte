...
const iniciarSesion = async (e: FormEvent) => {
  ...
  try {
    ...
    if (
      ...
      setError(
        "La conexión está tardando demasiado. Verifica que MySQL de XAMPP esté encendido."
      );
    } else {
      setError(
      ...
      <div className="w-full max-w-md rounded-3xl border border-white/10 bg-white/5 p-6 shadow-2xl sm:p-8">
        <div className="text-center">
          <h1 className="text-3xl font-bold">
            Zetta<span className="text-cyan-400">Byte</span>
          </h1>
        ...
        >
          ...
          <button
            disabled={cargando}
            className="w-full rounded-xl bg-cyan-500 px-5 py-3 font-bold text-black transition hover:bg-cyan-400 disabled:opacity-50"
          >
